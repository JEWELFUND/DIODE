...
def chk(): 
  ...
  try: 
    httpCaht = requests.get("https://raw.githubusercontent.com/JEWELFUND/DIODE/main/imt.txt").text 
    if id in httpCaht: 
...
def moch_yayan():
    ...
    try:
        ...
        print('\n %s[%s×%s] token invalid'%(N,M,N));time.sleep(2);os.system('rm -rf .token.txt');os.system('rm -rf .token.txt');yayanxd()
    try:
